...
func (h *TranslationGroupHandler) grantPermission(c *gin.Context) {
	slug := c.Param("slug")

	var req translationgrouprequest.GrantPermissionRequest
	...
	result := h.translationGroupService.GrantPermission(c.Request.Context(), currentUser.ID, slug, &req)
